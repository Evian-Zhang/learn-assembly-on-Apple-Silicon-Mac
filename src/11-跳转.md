...
下面举一个例子。我们有一个C语言代码（可以在本GitHub仓库`codes/`目录下的`11-likely.c`文件中看到）：
...
我们在学习C语言的过程中，一定看过一个说法：`switch`语句的效率比多个`if`-`else`语句串在一起要高。那究竟为什么会这样呢？我们不妨写一个C语言的程序（源码位于本GitHub仓库的`codes/`目录下的`11-switch.c`文件）：
